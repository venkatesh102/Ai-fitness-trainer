...
class Module:
    ...
    def findAngle(self, img, landmark1, landmark2, landmark3, draw=True):
...
text_to_speech('Press 2 for Left Bicep Curl')
text_to_speech('Press 3 for Pushups')
text_to_speech('Press 4 for Squats')
text_to_speech('Press 5 for Skipping')
text_to_speech('Press 6 to exit')
...
def right_bicep_curl():
    ...
    while True:
        ...
        fps = 1 / (cTime - pTime)
        pTime = cTime
        cv2.putText(img, str(int(fps)), (11, 50), cv2.FONT_HERSHEY_PLAIN, 5, (255, 0, 0), 5)
